...
const TaxFormEdit = ({
    ...
    console.log("ASASC", state)

    const handleSubmit = (values, { resetForm }) => {
        values.rate = values.rate / 1000
        values.applicable_items = selectedItems
        console.log(values);
        if (values.applicable_items && values.name && values.rate && values.applied_to) {
            const taxResources = localStorage.getItem("taxResources")
                ? JSON.parse(localStorage.getItem("taxResources"))
                : [];

            const newTaxResources = taxResources.filter((e) => {
                return values.name != e.name && values.rate != e.rate
            });

            localStorage.setItem("taxResources", JSON.stringify([...newTaxResources, values]))
        }
    };
    const [checked, setChecked] = useState([]);

    const [selectedItems, setSelectedItems] = useState(state.applicable_items);
    ...
    return (
        <Formik
            initialValues={{
                name: state.name,
                rate: state.rate,
                applied_to: state.applied_to,
                applicable_items: state.applicable_items,
            }}
        ...
        >
            {({ values, handleChange }) => (
                <Form style={{ padding: "5%" }}>
                    ...
                    <div style={{ padding: '5%' }}>
                        {Object.entries(groupedData).map(([categoryName, items]) => (
                            <div key={categoryName}>
                                <FormControlLabel style={{ backgroundColor: "#C8C8C8", width: "100%" }}
                                    control={
                                        <Checkbox
                                            ...
                                            onChange={() => {
                                                if (items.every((item) => selectedItems.includes(item.id))) {
                                                    ...
                                                    setSelectedItems([...selectedItems, ...items.map((item) => item.id)]);
                                                }
                                            }}
                                        />
                                    }
                    ...
                    }} style={{ float: "right", margin: "2%" }} type="submit" variant="contained" color="primary">
